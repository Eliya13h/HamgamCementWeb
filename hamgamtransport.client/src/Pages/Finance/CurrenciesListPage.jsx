...
function CurrenciesListPage() {
  ...
  const openEdit = useCallback((row) => {
    ...
    setEditForm({
      ...
      useInBothSystems: row.useInBothSystems ?? false,
      baseUnitsPerUnit: '',
      changeReason: '',
    })
  }, [])
  ...
  const handleCreateSubmit = async (event) => {
    ...
    try {
      const payload = {
        ...
        useInBothSystems: createForm.useInBothSystems,
      }
  ...
  const handleEditSubmit = async (event) => {
    ...
    try {
      await updateCurrency(editRow.currencyId, {
        ...
        useInBothSystems: editForm.useInBothSystems,
      })
      closeModals()
      await loadBaseCurrency()
      reloadTable()
  ...
  const tableOptions = useMemo(
    () => ({
      ...
      columns: [
        ...
        {
          data: 'useInBothSystems',
          name: 'useInBothSystems',
          render: (data) =>
            data
              ? '<span class="badge badge-active">هر دو سیستم</span>'
                    ...
                    <div className="col-12">
                      <div className="form-check form-switch">
                        <input
                          className="form-check-input"
                          type="checkbox"
                          id="create-currency-use-both"
                          checked={createForm.useInBothSystems}
                          onChange={(e) =>
                            setCreateForm((p) => ({ ...p, useInBothSystems: e.target.checked }))
                          }
                        />
                        <label className="form-check-label" htmlFor="create-currency-use-both">
                    ...
                    <div className="col-12">
                      <div className="form-check form-switch">
                        <input
                          className="form-check-input"
                          type="checkbox"
                          id="edit-currency-use-both"
                          checked={editForm.useInBothSystems}
                          onChange={(e) =>
                            setEditForm((p) => ({ ...p, useInBothSystems: e.target.checked }))
                          }
                        />
                        <label className="form-check-label" htmlFor="edit-currency-use-both">
                          استفاده در هر دو سیستم (سیمان + ترانسپورت)
                        </label>
                      </div>
                    </div>
