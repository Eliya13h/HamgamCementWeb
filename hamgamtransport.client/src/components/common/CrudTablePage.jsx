...
function formToPayload(fields, form) {
  const payload = {}
  for (const field of fields) {
    if (field.skipOnSubmit || field.omitFromPayload || field.autoCode) {
      continue
    }
...
/**
 * صفحه CRUD مشترک — جدول سرور-ساید + مودال‌های ایجاد/ویرایش/حذف
 */
  ...
  const handleFieldChange = useCallback(
    (name, value) => {
      setForm((prev) => {
        ...
        const patch = onFormChange?.(name, value, next)
        if (patch && typeof patch.then === 'function') {
          patch.then((asyncPatch) => {
            if (asyncPatch && typeof asyncPatch === 'object') {
              setForm((current) => ({ ...current, ...asyncPatch }))
            }
          })
        } else if (patch) {
          next = { ...next, ...patch }
