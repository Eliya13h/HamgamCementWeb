...
import {
  createVehicleOwner,
  createVehicleOwnersDataTableAjax,
  deleteVehicleOwner,
  updateVehicleOwner,
} from '../../services/vehicleOwnersApi'
...
function VehicleOwnersPage() {
  const tableRef = useRef(null)
  const { canCreate, canEdit, canDelete } = usePageCrud('/people/vehicle-owners')
  ...
  const handleCreateSubmit = async (event) => {
    ...
    try {
      await createVehicleOwner(buildPayload(createForm))
      closeModals()
      reloadTable()
    } catch (error) {
  ...
  const handleEditSubmit = async (event) => {
    ...
    try {
      await updateVehicleOwner(editRow.vehicleOwnerId, buildPayload(editForm))
      closeModals()
      reloadTable()
    } catch (error) {
      setFormError(error.message)
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      await deleteVehicleOwner(deleteRow.vehicleOwnerId)
      closeModals()
      reloadTable()
    } catch (error) {
      setFormError(error.message)
  ...
  const tableOptions = useMemo(
    () => ({
      processing: true,
      serverSide: true,
      ajax: createVehicleOwnersDataTableAjax(setLoadError),
      paging: true,
      searching: true,
      ordering: true,
      info: true,
    ...
    <div className="users-page">
      <div className="content-card card border-0 h-100">
        <div className="card-header bg-transparent border-0 pt-4 px-4 pb-0 d-flex align-items-center justify-content-between gap-3 flex-wrap">
          <h2 className="card-title mb-0">موترداران</h2>
          {canCreate && (
            <button
              type="button"
            ...
            <div className="modal-dialog modal-dialog-centered modal-dialog-scrollable">
              <form className="modal-content" onSubmit={handleCreateSubmit}>
                <div className="modal-header">
                  <h5 className="modal-title">موتردار جدید</h5>
                  <button type="button" className="btn-close" aria-label="بستن" onClick={closeModals} />
                </div>
                <div className="modal-body">
                  {formError && <div className="alert alert-danger py-2">{formError}</div>}
                  <PersonFormFields form={createForm} setForm={setCreateForm} idPrefix="create-vehicle-owner" />
                </div>
                <div className="modal-footer">
                  ...
                  <button type="submit" className="btn btn-accent" disabled={submitting}>
                    {submitting ? 'در حال ایجاد...' : 'ایجاد موتردار'}
                  </button>
                </div>
              </form>
            </div>
          ...
          <div className="modal show d-block users-modal" tabIndex="-1" role="dialog">
            <div className="modal-dialog modal-dialog-centered modal-dialog-scrollable">
              <form className="modal-content" onSubmit={handleEditSubmit}>
                <div className="modal-header">
                  <h5 className="modal-title">ویرایش موتردار</h5>
                  <button type="button" className="btn-close" aria-label="بستن" onClick={closeModals} />
                </div>
                <div className="modal-body">
                  {formError && <div className="alert alert-danger py-2">{formError}</div>}
                  <PersonFormFields form={editForm} setForm={setEditForm} idPrefix="edit-vehicle-owner" />
                </div>
                <div className="modal-footer">
                  <button type="button" className="btn btn-outline-secondary" onClick={closeModals}>
                    انصراف
          ...
          <div className="modal show d-block users-modal" tabIndex="-1" role="dialog">
            <div className="modal-dialog modal-dialog-centered modal-dialog-scrollable">
              <div className="modal-content">
                <div className="modal-header">
                  <h5 className="modal-title">حذف موتردار</h5>
                  <button type="button" className="btn-close" aria-label="بستن" onClick={closeModals} />
                </div>
                <div className="modal-body">
                  {formError && <div className="alert alert-danger py-2">{formError}</div>}
                  <p className="mb-0">
                    آیا از حذف موتردار <strong>{deleteRow.fullName}</strong> اطمینان دارید؟
                  </p>
                </div>
                <div className="modal-footer">
                  <button type="button" className="btn btn-outline-secondary" onClick={closeModals}>
